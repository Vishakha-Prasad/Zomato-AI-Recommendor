...
class TestCatalog:
    ...
    def test_get_locations_returns_list(self):
        from phase_2_backend_api.backend.catalog import get_locations
        locs = get_locations()
    ...
    def test_get_cuisines_returns_list(self):
        from phase_2_backend_api.backend.catalog import get_cuisines
    ...
    def test_filter_restaurants_empty_filters_returns_rows(self):
        from phase_2_backend_api.backend.catalog import filter_restaurants
    ...
    def test_filter_restaurants_by_location(self):
        from phase_2_backend_api.backend.catalog import get_locations, filter_restaurants
        locs = get_locations()
        if locs:
            loc = locs[0]
